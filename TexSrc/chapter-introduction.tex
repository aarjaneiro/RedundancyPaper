...
\begin{definition}[Threshold$(R,d)$]
    \hfill \\
    \textbf{Threshold(R,d)}, denoted by \normalfont{$\mathcal{D}_{\text{Thresh}(R,d,Z)}$}, selects $d$ queues upon a job arrival,
    which:
    \begin{enumerate}
...
\begin{conjecture}
    \hfill \\
    As $N \longrightarrow \infty$, the system \normalfont{$\mathcal{D}_{\text{Thresh}(R,d)}$} becomes $(\psi, \Gamma)$-exchangeable.
