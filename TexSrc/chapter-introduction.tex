...
Take for example \textit{Redudancy-d}, wherein $d$ servers are chosen per arrival;
...
\begin{definition}[Workload]
    ...
    \end{align}
    for counting measure $\#$, given that $\#(X^{-1}_{i}): t \longmapsto z$, (or that a queue has countable jobs). \textbf{System Load} refers to the amount of work remaining in the entire system,
    \begin{align*}
    ...
    As such, $\psi$ will refer to this more-general case from henceforth. 
\end{definition}
As an example, a service time in a $G/M/c$ system will be drawn from an exponential distribution. In this simple case, the mth arriving job can be given the ``marks'' $(T_{m}, S_{m})\equiv (T, S)_{m}$ where $S_{m} \overset{IID}\sim \text{EXP}(\lambda)$ and $T_{m}$ is the time of arrival; conditioning on the process $(T,S)_{m}$, $W_{i}(t)$ turns into a matter of merely adding up the enqueued service times and that remaining of the currently serviced job, which is conveniently memoryless.

Altogether, Figure~\ref{fig:1} describes the behaviour which would be expected in an ideal system wherein both a mean field and asymptotic independence can be achieved~\cite{mukhopadhyay_analysis_nodate}. In particular, $P$ describes a fixed point, giving the collection a distribution of $\delta_{P}$.
...
\begin{definition}[Threshold(R,d)]
    ...
    Next:
    \begin{enumerate}
        \item For $i \leq d$ queues which have workload less than or equal to $R$, place copies in these $i$.
