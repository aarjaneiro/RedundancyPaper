...
\begin{definition}[HSIC Testing for Two Non-stationary Processes]
\label{hsichat}
...
where $P_{Z}$ denotes the marginal distribution of $Z$,~\cite{nonstat} suggest a resampling procedure.
    Specifically, for any estimator of $\text{HSIC}(XY)$, $\hat \text{HSIC}(XY)$, sample a common set of $N_{\text{time}}$ times, $\{\tau_{\ell}\}_{ell \leq N_{\text{time}}} \subset T$, and calculate $\hat H :=\{\hat\text{HSIC}(X_{(\{\tau_{N_{\text{time}}}\})}Y_{\gamma(\tau_{N_{\text{time}}})}))\}_{\gamma \in \Gamma}$ where $X_{(\{\tau_{N_{\text{time}}}\})}$ denotes $X$ restricted to the sampled times $\{\tau_{N_{\text{time}}}\}$ and $\Gamma$ is a set of permutations on these $N_{\text{time}}$ samples.
    After taking $c_{\alpha} := \hat H_{(1-\alpha)N_{\text{times}}}$, the null hypothesis is rejected if $\hat \text{HSIC}(XY) > c_{\alpha}$
\end{definition}
...
Unlike the procedure utilized by~\cite{nonstat}, however, we adopt a modification
